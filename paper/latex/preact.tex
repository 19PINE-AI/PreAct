...
\end{scope}
\end{tikzpicture}
\caption{Cold-to-warm SR deltas under verify-gate ON vs OFF, on AndroidWorld official-15 ($n{=}5$ seeds) and OSWorld test\_tiny ($n{=}5$ reps). \textbf{The diff-of-deltas is identically 2.6 tasks on both platforms} (and 3 tasks on the WebArena 12-task subset, $n{=}2{+}2$ symmetric; \S\ref{sec:eval-webarena}), despite different LLM backends (Gemini 3 Flash on Android, Claude Sonnet 4.6 on OSWorld and WebArena) and different task subsets. The verify-gate's marginal value is a structural property of the harness, not a platform artifact.}
