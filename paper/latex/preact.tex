...
We start from the artifact, because in PreAct the artifact \emph{is} the runtime. When the agent first succeeds at ``add Jordan Chen,'' PreAct compiles the trace into the program sketched in Listing~\ref{lst:contact} (the full JSON is in Appendix~A). It is a small graph: \emph{states} carry a verification predicate---an accessibility-tree pattern that must hold on the live screen for the agent to believe it is in that state---and \emph{transitions} carry an action. Input values seen in the trace that match the task description (the name, the phone number) are lifted to \emph{parameters} (\texttt{\$first}, \texttt{\$phone}, \dots), so the one program serves every future ``add a contact'' request.

\begin{lstlisting}[caption={A compiled program for ``add a contact'' (abbreviated). States carry verification predicates ($\models$); transitions carry actions. This graph is executed directly---it is not regenerated into a flat script.},label={lst:contact},captionpos=b,basicstyle=\ttfamily\footnotesize,language={}]
states                                          parameters: $first $last $phone $label
  home          |= launcher visible
  contacts_main |= package == com.android.contacts
  form_open     |= element resource_id == name_field
  form_filled   |= text == "$first $last"
  saved         |= (terminal)
transitions
  home          --open_app("Contacts")-------->  contacts_main
  contacts_main --tap("Create new contact")---->  form_open
  form_open     --type("$first $last")--------->  form_filled
  form_filled   --tap("Save")----------------->  saved      (+ phone, label steps)
\end{lstlisting}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig_program_graph.pdf}
\caption{The compiled ``add a contact'' program as a graph: states (boxes) hold a verification predicate that must match the live screen; transitions (arrows) hold an action. The harness executes this graph directly. Listing~\ref{lst:contact} is the same program in stored JSON form.}
