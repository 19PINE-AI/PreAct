...
Figure~\ref{fig:diff-of-deltas} previews the load-bearing finding: the verify-gate's marginal value (the cold-to-warm delta with the gate ON minus the same delta with it OFF) is identical 2.6 tasks on both Android and OSWorld, and 2.5 tasks on WebArena's 12-task subset.
...
\end{scope}
\end{tikzpicture}
\caption{Cold-to-warm SR deltas under verify-gate ON vs OFF, on AndroidWorld official-15 ($n{=}5$ seeds) and OSWorld test\_tiny ($n{=}5$ reps). \textbf{The diff-of-deltas is identically 2.6 tasks on both platforms} (and 2.5 tasks on the WebArena 12-task subset, $n{=}2$ gate-ON / $n{=}1$ gate-OFF; \S\ref{sec:eval-webarena}), despite different LLM backends (Gemini 3 Flash on Android, Claude Sonnet 4.6 on OSWorld and WebArena) and different task subsets. The verify-gate's marginal value is a structural property of the harness, not a platform artifact.}
\label{fig:diff-of-deltas}
\end{figure}

This is the load-bearing experiment. We run a 2$\times$2 ablation (cold/warm $\times$ verify-gate ON/OFF) at $n{=}5$ seeds on AndroidWorld, $n{=}5$ repetitions on OSWorld test\_tiny, and $n{=}2$ gate-ON / $n{=}1$ gate-OFF on WebArena's 12-task shopping\_admin subset.
...
\subsubsection{WebArena ($n{=}2$ gate-ON, $n{=}1$ gate-OFF, 12 shopping\_admin tasks)}
...
\textbf{Gate OFF (harness default, $n{=}1$).} Cold (Run 1) Eval SR: $4/12 = 33.3\%$. Warm (Run 2) Eval SR: \textbf{$0/12 = 0\%$}. \textbf{All 12 warm replays hit cov$=$100\%/score$=$0.0}---100\% smoking-gun reproducibility, the highest of any platform. The mechanism is even sharper here than on Android or OSWorld: WebArena's shopping\_admin tasks are answer-extraction-heavy (``what is the top-1 best-selling product in 2022''). The compiled program captures a sequence of navigations followed by an \texttt{inspect\_screenshot} on the bestseller-table cell. On Run 2's fresh browser state, the navigation replays correctly (cov$=$100\%) but the \texttt{inspect\_screenshot} returns whatever value happens to be there now---which the live evaluator scores against a string-match criterion from the run-1 snapshot. $\Delta_\text{OFF} = -4$ tasks ($-33$\,pp).

\textbf{Gate ON (patched harness, $n{=}2$ reps).} Rep-1: cold $5/12 = 42\%$, warm $4/12 = 33\%$, $\Delta = -1$. Rep-2: cold $6/12 = 50\%$, warm $4/12 = 33\%$, $\Delta = -2$. Mean $\Delta_\text{ON} = -1.5 \pm 0.71$. The warm SR is identical across both reps---4 verified replays of the review-counting program family (tasks 11--15, parametrized on the search term)---demonstrating that the gate selectively keeps the genuinely-faithful program while rejecting the 4--5 lossy compiles per rep that would have polluted gate-OFF's corpus. Verify-replay overhead per rep: median 5.0\,s on this benchmark (much smaller than CUA's 30--90\,s).

\textbf{Mechanism observed.} Across both gate-ON reps, the gate fired 8 $\Delta$replay-fail events (4--5 per rep) at compile time: bestseller-extraction tasks (T0, T3, T4, T5, T6) where the compiled \texttt{inspect\_screenshot} would return whatever value is on the live page rather than the run-1 snapshot. These are exactly the cov$=$100\%/score$=$0 cases that gate-OFF stores and replays at warm time. The gate observes them at compile time, before the corpus is polluted. The verified review-counting program ran at 2.8--3.5\,s per warm replay vs.\ 27--36\,s CUA cold ($8.5$--$13\times$ speedup).

\textbf{Diff-of-deltas: mean$_\text{ON} - \Delta_\text{OFF} = -1.5 - (-4) = 2.5$ tasks (21\,pp on the 12-task subset).} Combined with Android ($n{=}5$, diff-of-deltas 2.6 tasks / 17.3\,pp) and OSWorld ($n{=}5$, 2.6 tasks / 43\,pp), WebArena's 2.5 tasks / 21\,pp extends the gate-necessity claim to three structurally different platforms, three LLM backends, three task evaluation paradigms (Android end-state UI predicates, OSWorld desktop-state evaluators, WebArena string-match on extracted answers). The smoking-gun reproducibility (compile-time, gate-ON) scales monotonically with how state-side-effect-dependent the evaluator is: from 2/5 (Android) through 3--5/5 (OSWorld) up to 4--5/6 (WebArena, where every CUA-success-on-bestseller-task was caught). When the program does pass the gate---one with a deterministic answer-extraction path---the gate is selective rather than aggressive: it keeps the program and the harness reaps the speedup.
...
2 & Verify-gate ablation & \textbf{Closed cross-platform on 3 platforms} & Android $n{=}5$ ($p<0.001$); OSWorld $n{=}5$ (43\,pp); WebArena $n{=}2{+}1$ (21\,pp) \\
