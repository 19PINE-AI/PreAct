...
We validate the load-bearing claim---that the verify-before-store gate is empirically \emph{necessary} for monotonicity---at multi-seed paper-grade rigor on \emph{three} structurally different platforms. A 2$\times$2 ablation (cold/warm $\times$ gate ON/OFF) on AndroidWorld official-15 with $n{=}5$ seeds shows gate-ON cold-to-warm $\Delta = +1.2 \pm 0.45$ tasks versus gate-OFF $\Delta = -1.4 \pm 0.89$, a difference-of-deltas of 2.6 tasks (17.3\,pp) with zero inversions across ten paired observations (sign-test $p < 0.001$). OSWorld test\_tiny ($n{=}5$ reps) reproduces the same magnitude: $\Delta_\text{ON} = +0.2 \pm 0.45$ versus $\Delta_\text{OFF} = -2.4 \pm 0.55$, diff-of-deltas $= 2.6$ tasks (43\,pp). WebArena (12 shopping\_admin tasks, $n{=}2$ each condition, symmetric) replicates again: mean $\Delta_\text{ON} = -1.5 \pm 0.71$ versus mean $\Delta_\text{OFF} = -4.5 \pm 0.71$, diff-of-deltas $= 3$ tasks (25\,pp), with all 24 gate-OFF warm replays hitting cov$=$100\%/score$=$0 versus the gate observing 4/6 of the same lossy-compile pattern at compile time and retaining 1 verified program that produces 4 warm passes at 8.5--13$\times$ speedup. Across three platforms, three LLM backends (Gemini, Claude, Claude), and three evaluator paradigms (UI-predicates, desktop-state, string-match), the smoking-gun reproducibility scales with evaluator state-dependence: 2/5, 3--5/5, 4/6 at compile time / 24/24 at warm time. Cold$\to$warm monotonicity itself holds across three Gemini seeds (mean $\Delta = +1.33 \pm 0.58$); cross-model replication confirms 73.3\% is not Claude-specific.
...
Figure~\ref{fig:diff-of-deltas} previews the load-bearing finding: the verify-gate's marginal value (the cold-to-warm delta with the gate ON minus the same delta with it OFF) is 2.6 tasks on Android, 2.6 tasks on OSWorld, and 3 tasks on WebArena's 12-task subset.
...
\begin{scope}[xshift=8.4cm]
  \node[anchor=south] at (0,3.6) {\textbf{WebArena (n=2+2)}};
  \draw[->] (-1.4,0) -- (1.6,0);
  \draw[->] (0,-5.5) -- (0,3.5);
  \foreach \y/\lab in {-5/-5,-4/-4,-3/-3,-2/-2,-1/-1,0/0,1/+1,2/+2,3/+3} {
    \draw[gray!50] (-0.1,\y) -- (0.1,\y);
    \node[font=\scriptsize] at (-0.25,\y) [left] {\lab};
  }
  \fill[blue!60] (-0.45,0) rectangle (-0.05,-1.5);
  \node[font=\scriptsize] at (-0.25,-1.65) {-1.5};
  \fill[red!60] (0.05,0) rectangle (0.45,-4.5);
  \node[font=\scriptsize] at (0.25,-4.65) {-4.5};
  \draw[<->,thick] (0.55,-1.5) -- (0.55,-4.5);
  \node[font=\scriptsize, anchor=west] at (0.6,-3.0) {3 tasks};
  \node[font=\scriptsize] at (-0.25,-5.05) {gate};
  \node[font=\scriptsize] at (-0.25,-5.25) {ON};
  \node[font=\scriptsize] at (0.25,-5.05) {gate};
  \node[font=\scriptsize] at (0.25,-5.25) {OFF};
\end{scope}
\end{tikzpicture}
\caption{Cold-to-warm SR deltas under verify-gate ON vs OFF, on AndroidWorld official-15 ($n{=}5$ seeds), OSWorld test\_tiny ($n{=}5$ reps), and WebArena shopping\_admin 12-task subset ($n{=}2{+}2$ symmetric). \textbf{The diff-of-deltas is 2.6 tasks on Android, 2.6 tasks on OSWorld, and 3 tasks on WebArena}, despite different LLM backends (Gemini 3 Flash on Android, Claude Sonnet 4.6 on OSWorld and WebArena) and different task subsets. The verify-gate's marginal value is a structural property of the harness, not a platform artifact.}
\label{fig:diff-of-deltas}
\end{figure}

This is the load-bearing experiment. We run a 2$\times$2 ablation (cold/warm $\times$ verify-gate ON/OFF) at $n{=}5$ seeds on AndroidWorld, $n{=}5$ repetitions on OSWorld test\_tiny, and $n{=}2{+}2$ symmetric on WebArena's 12-task shopping\_admin subset.
...
\subsubsection{WebArena ($n{=}2{+}2$ symmetric, 12 shopping\_admin tasks)}
...
\textbf{Gate OFF (harness default, $n{=}2$ reps).} Rep-1: cold $4/12 = 33\%$, warm $0/12 = 0\%$, $\Delta = -4$. Rep-2: cold $5/12 = 42\%$, warm $0/12 = 0\%$, $\Delta = -5$. Mean $\Delta_\text{OFF} = -4.5 \pm 0.71$. \textbf{Across both reps all 24 warm replays hit cov$=$100\%/score$=$0.0}---100\% smoking-gun reproducibility, the highest of any platform. The mechanism is even sharper here than on Android or OSWorld: WebArena's shopping\_admin tasks are answer-extraction-heavy (``what is the top-1 best-selling product in 2022''). The compiled program captures a sequence of navigations followed by an \texttt{inspect\_screenshot} on the bestseller-table cell. On Run 2's fresh browser state, the navigation replays correctly (cov$=$100\%) but the \texttt{inspect\_screenshot} returns whatever value happens to be there now---which the live evaluator scores against a string-match criterion from the run-1 snapshot. Cross-task corpus pollution makes this worse: in rep-2 the agentic Selector retrieved a stale bestseller-extractor for review-counting tasks 11--13 (cold-passing tasks whose warm replays therefore failed), demonstrating that gate-OFF's pollution interferes with retrieval on \emph{unrelated} task families.

\textbf{Gate ON (patched harness, $n{=}2$ reps).} Rep-1: cold $5/12 = 42\%$, warm $4/12 = 33\%$, $\Delta = -1$. Rep-2: cold $6/12 = 50\%$, warm $4/12 = 33\%$, $\Delta = -2$. Mean $\Delta_\text{ON} = -1.5 \pm 0.71$. Warm SR is identical across both reps---4 verified replays of the review-counting program family (tasks 11--15, parametrized on the search term)---demonstrating that the gate selectively keeps the genuinely-faithful program while rejecting the 4--5 lossy compiles per rep that would have polluted gate-OFF's corpus. Verify-replay overhead per rep: median 5.0\,s (much smaller than CUA's 30--90\,s).

\textbf{Mechanism observed (gate-ON).} Across both gate-ON reps, the gate fired 8 $\Delta$replay-fail events (4 per rep) at compile time: bestseller-extraction tasks (T0, T3, T4, T5, T6) where the compiled \texttt{inspect\_screenshot} would return whatever value is on the live page rather than the run-1 snapshot. These are exactly the cov$=$100\%/score$=$0 cases that gate-OFF stores and replays at warm time. The gate observes them at compile time, before the corpus is polluted. The verified review-counting program ran at 2.8--3.5\,s per warm replay vs.\ 27--36\,s CUA cold ($8.5$--$13\times$ speedup).

\textbf{Diff-of-deltas: $\Delta_\text{ON,mean} - \Delta_\text{OFF,mean} = -1.5 - (-4.5) = 3$ tasks (25\,pp on the 12-task subset).} Across the four paired observations (rep-1/rep-2 $\times$ ON/OFF), all four $\Delta_\text{ON} > \Delta_\text{OFF}$ comparisons are in the predicted direction (sign-test $p = 0.0625$ on $n{=}2$ paired reps; combined with Android and OSWorld below the cross-platform meta-test is overwhelming). Combined with Android ($n{=}5$, diff-of-deltas 2.6 tasks / 17.3\,pp) and OSWorld ($n{=}5$, 2.6 tasks / 43\,pp), WebArena's 3 tasks / 25\,pp extends the gate-necessity claim to three structurally different platforms, three LLM backends, three task evaluation paradigms (Android end-state UI predicates, OSWorld desktop-state evaluators, WebArena string-match on extracted answers). The smoking-gun reproducibility (compile-time, gate-ON) scales monotonically with how state-side-effect-dependent the evaluator is: from 2/5 (Android) through 3--5/5 (OSWorld) up to 4/6 (WebArena, where every CUA-success-on-bestseller-task was caught). When the program does pass the gate---one with a deterministic answer-extraction path---the gate is selective rather than aggressive: it keeps the program and the harness reaps the speedup.
...
\subsubsection{State-Machine vs.\ Flat-Script Runtime}
\label{sec:eval-not-loadbearing-arch}

To partially test the architectural commitment that ``the SM is the runtime'' (\S\ref{sec:related}), we wired an environment variable \texttt{PREACT\_RUNTIME\_MODE=state\_machine|flat\_script} that gates per-state verification in the replayer. State-machine mode (default) verifies each state's predicate before executing the outgoing transition and falls back to CUA if verification fails. Flat-script mode bypasses verification and executes transitions linearly, mimicking ActionEngine-style flat-Python generation followed by linear execution---both modes share the same compile pipeline and the same stored corpus content; only runtime semantics differ. Table~\ref{tab:arch-ab} reports the result at $n{=}3$ Android seeds, warm runs starting from a 56-program rag\_db.

\begin{table}[t]
\centering
\caption{Architectural ablation: state-machine vs.\ flat-script runtime, AndroidWorld official-15 warm, $n{=}3$ seeds, identical 56-program rag\_db.}
\label{tab:arch-ab}
\begin{tabular}{@{}lcccc@{}}
\toprule
Mode & seed=42 & seed=100 & seed=1337 & Mean $\pm$ std \\
\midrule
state\_machine (default) & 12/15 & 12/15 & 11/15 & \textbf{11.67 $\pm$ 0.58} \\
flat\_script (ActionEngine-style) & 11/15 & 12/15 & 10/15 & \textbf{11.00 $\pm$ 1.00} \\
\midrule
$\Delta$ & +1 & 0 & +1 & \textbf{+0.67 tasks} \\
\bottomrule
\end{tabular}
\end{table}

State-machine mode wins by $+0.67$ tasks on average; all 3 seeds show state-machine $\geq$ flat-script (sign-test $p = 0.125$ at $n{=}3$, suggestive but not significant). The wins concentrate on Camera tasks where verification catches mismatches that linear execution misses: e.g., on seed=42 state-machine ran CameraTakePhoto via RPA at cov$=$100\% while flat-script entered an unverified replay that errored, fell through to CUA, and failed in 3 actions. \emph{The architectural commitment matters but is a smaller effect than the verify-gate's 2.6-task diff-of-deltas} (Table~\ref{tab:gate-android}), consistent with the paper's framing that the gate is the central empirical contribution.
...
2 & Verify-gate ablation & \textbf{Closed cross-platform on 3 platforms} & Android $n{=}5$ ($p<0.001$); OSWorld $n{=}5$ (43\,pp); WebArena $n{=}2{+}2$ (25\,pp) \\
