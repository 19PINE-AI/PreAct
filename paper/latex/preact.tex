...
\subsection{Out-of-Distribution Generalization (Partial)}
\label{sec:eval-ood}

To test whether the corpus transfers beyond seen tasks, we built a rag\_db from the OSWorld test\_tiny set (6 tasks across Chrome, LibreOffice Calc, LibreOffice Writer, with 4 verify-gate-stored programs) and ran on test\_small (39 tasks across 10 domains). On the first 11 tasks (mix of in-distribution and adjacent tasks), the OOD-warm run achieved 10/11 (91\%) vs.\ a fresh-cold baseline of 11/11 (100\%) on the same prefix---\textbf{the corpus HURT by at least one task}. The single failure (a LibreOffice Calc task) involved 44 actions with cov$=$0\% replay coverage, suggesting the selector retrieved a non-matching test\_tiny Calc program and the agent wasted budget on an inapplicable replay. The full 39-task run was not completed due to a harness-level hang on task 12. We treat this as suggestive evidence that the corpus does not transfer cleanly to OOD tasks; the corpus's value (\S\ref{sec:eval-monotonic}) appears specific to repeated invocations of seen tasks rather than generalization across task domains. Multi-seed completion of this experiment is future work.
