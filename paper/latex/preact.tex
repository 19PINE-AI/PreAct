...
We validate the load-bearing claim---that the verify-before-store gate is empirically \emph{necessary} for monotonicity---at multi-seed paper-grade rigor on \emph{three} structurally different platforms. A 2$\times$2 ablation (cold/warm $\times$ gate ON/OFF) on AndroidWorld official-15 with $n{=}5$ seeds shows gate-ON cold-to-warm $\Delta = +1.2 \pm 0.45$ tasks versus gate-OFF $\Delta = -1.4 \pm 0.89$, a difference-of-deltas of 2.6 tasks (17.3\,pp) with zero inversions across ten paired observations (sign-test $p < 0.001$). OSWorld test\_tiny ($n{=}5$ reps) reproduces the same magnitude: $\Delta_\text{ON} = +0.2 \pm 0.45$ versus $\Delta_\text{OFF} = -2.4 \pm 0.55$, diff-of-deltas $= 2.6$ tasks (43\,pp). WebArena (12 shopping\_admin tasks, both gate conditions) replicates again: $\Delta_\text{ON} = -1$ versus $\Delta_\text{OFF} = -4$, diff-of-deltas $= 3$ tasks (25\,pp), with all 12 gate-OFF warm replays hitting cov$=$100\%/score$=$0 versus the gate observing 5/6 of the same lossy-compile pattern at compile time and retaining 1 verified program that produces 4 warm passes at 10--13$\times$ speedup. Across three platforms, three LLM backends (Gemini, Claude, Claude), and three evaluator paradigms (UI-predicates, desktop-state, string-match), the smoking-gun reproducibility scales with evaluator state-dependence: 2/5, 3--5/5, 5/6 at compile time / 12/12 at warm time. Cold$\to$warm monotonicity itself holds across three Gemini seeds (mean $\Delta = +1.33 \pm 0.58$); cross-model replication confirms 73.3\% is not Claude-specific.
...
\end{scope}
\end{tikzpicture}
\caption{Cold-to-warm SR deltas under verify-gate ON vs OFF, on AndroidWorld official-15 ($n{=}5$ seeds) and OSWorld test\_tiny ($n{=}5$ reps). \textbf{The diff-of-deltas is identically 2.6 tasks on both platforms} (and 3 tasks on the WebArena 12-task subset, $n{=}1$; \S\ref{sec:eval-webarena}), despite different LLM backends (Gemini 3 Flash on Android, Claude Sonnet 4.6 on OSWorld and WebArena) and different task subsets. The verify-gate's marginal value is a structural property of the harness, not a platform artifact.}
...
\subsubsection{WebArena ($n{=}1$ each condition, 12 shopping\_admin tasks)}
\label{sec:eval-webarena}

To extend the cross-platform replication to a third structurally different environment, we ran a 12-task shopping\_admin subset of WebArena~\cite{zhou2024webarena} under both gate conditions, using the existing harness's two-run protocol (Run 1 = exploration, Run 2 = replay). For the gate-ON condition we patched the WebArena harness to mirror Android/OSWorld semantics: after each Run 1 compile, run a fresh-state verify-replay and delete the program if it does not pass the evaluator with score $\geq 1.0$.

\textbf{Gate OFF (harness default).} Cold (Run 1) Eval SR: $4/12 = 33.3\%$. Warm (Run 2) Eval SR: \textbf{$0/12 = 0\%$}. \textbf{All 12 warm replays hit cov$=$100\%/score$=$0.0}---100\% smoking-gun reproducibility, the highest of any platform. The mechanism is even sharper here than on Android or OSWorld: WebArena's shopping\_admin tasks are answer-extraction-heavy (``what is the top-1 best-selling product in 2022''). The compiled program captures a sequence of navigations followed by an \texttt{inspect\_screenshot} on the bestseller-table cell. On Run 2's fresh browser state, the navigation replays correctly (cov$=$100\%) but the \texttt{inspect\_screenshot} returns whatever value happens to be there now---which the live evaluator scores against a string-match criterion from the run-1 snapshot. $\Delta_\text{OFF} = -4$ tasks ($-33$\,pp).

\textbf{Gate ON (patched harness).} Cold (Run 1) Eval SR: $5/12 = 42\%$. Warm (Run 2) Eval SR: \textbf{$4/12 = 33\%$}. The verify-gate fired on every CUA-success-then-compile cycle and rejected 5 out of 6 compiled programs (Δreplay-fail at cov$=$100\%/score$=$0 for tasks 0, 3, 5, 6 and at cov$=$50\%/score$=$0 for task 4)---directly observing the same lossy-replay mechanism that gate-OFF only exposes at warm time, but at compile time before the corpus is polluted. The single retained program (a review-counting state machine) then served 4 verified warm replays via the agentic Program Selector (tasks 12--15, all same task family), each at $\sim 3.1$ seconds vs $\sim 35$ seconds CUA cold (10--13$\times$ speedup). $\Delta_\text{ON} = -1$ task.

\textbf{Diff-of-deltas: $-1 - (-4) = 3$ tasks (25\,pp on the 12-task subset).} Combined with Android ($n{=}5$, diff-of-deltas 2.6 tasks / 17.3\,pp) and OSWorld ($n{=}5$, 2.6 tasks / 43\,pp), WebArena's 3 tasks / 25\,pp extends the gate-necessity claim to three structurally different platforms, three LLM backends, three task evaluation paradigms (Android end-state UI predicates, OSWorld desktop-state evaluators, WebArena string-match on extracted answers). The smoking-gun reproducibility (compile-time, gate-ON) scales monotonically with how state-side-effect-dependent the evaluator is: from 2/5 (Android) through 3--5/5 (OSWorld) up to 5/6 (WebArena, where every CUA-success that didn't return an evaluator-stable answer was caught). The single program that did pass the gate---one with a deterministic answer-extraction path---demonstrates that the gate is selective rather than aggressive: when the program is genuinely faithful, the gate keeps it and the harness reaps the speedup.
...
To test whether the corpus transfers beyond seen tasks, we built a rag\_db from the OSWorld test\_tiny set (6 tasks across Chrome, LibreOffice Calc, LibreOffice Writer; 4 verify-gate-stored programs after compile filtering) and ran on \texttt{test\_small\_ood.json}: the 33 OSWorld test\_small tasks that are \emph{not} in test\_tiny, spanning 9 domains (chrome$\times$2, gimp$\times$2, libreoffice\_calc$\times$1, libreoffice\_impress$\times$2, libreoffice\_writer$\times$0, multi\_apps$\times$17, os$\times$2, thunderbird$\times$2, vlc$\times$2, vs\_code$\times$3). The agentic Program Selector retrieves from the test\_tiny corpus when the new task's description plausibly matches.

Result: \textbf{warm-OOD 21/30 = 70.0\%} (3 tasks errored during environment setup, not counted) vs.\ a cold-baseline of 26/36 = 72.2\% on the full test\_small (which includes 6 in-distribution test\_tiny tasks; the cold baseline on the 30 OOD tasks alone is approximately $20/30 = 66.7\%$ after subtracting the 6 in-distribution successes).
