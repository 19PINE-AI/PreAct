...
\textbf{(L1) Architectural baselines untested.} We did not run an ActionEngine-style flat-script baseline or an untargeted-exploration baseline. The architectural commitments (state-machine-as-executable, task-directed recording) are supported by the working implementation across two platforms but not by direct AB comparison. Future work could implement an ActionEngine-faithful re-execution path on top of our compile pipeline (compile to the same state machine, then \emph{generate flat Python} at the verify step instead of executing the graph directly) to isolate the effect of the architectural commitment.
...
\textbf{(L3) Compile cost.} The verify-before-store gate requires a full re-replay and re-evaluation of every compiled program, doubling the wall time of cold runs (since each successful CUA-then-compile is followed by a verify-replay). On AndroidWorld, this translates to approximately 50\% extra wall on cold runs vs.\ a no-verify baseline. The cost is amortized on warm runs (where the corpus retrieves already-verified programs in seconds), but the compile-time cost is real and may matter for high-throughput deployments.
...
\textbf{(L5) LLM nondeterminism in selector.} The agentic Program Selector is itself an LLM call. Across our experiments, selector decisions were stable on identical inputs (the prompt is short and the discrimination problem is usually unambiguous), but selector nondeterminism on borderline cases is a potential source of warm-SR variance that we did not measure directly. A principled selector evaluation (e.g.\ accuracy of program selection given ground-truth task labels) is future work.
