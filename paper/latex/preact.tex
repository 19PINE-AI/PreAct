...
Equally informative are the \emph{negative} findings: code-level runtime guardrails are aggregate-neutral at $n{=}5$ (cold means identical, $10.2 = 10.2$); prompt-level Pre-Act content is dead code in the production CUA path; and an embedding-RAG selector baseline is empirically competitive with our agentic LLM selector ($87\%$ vs.\ $76\%$ functional retrieval at the same 0\% wrong-task rate). On WebArena's same-task replay protocol, a matched $n{=}4{+}4$ head-to-head against \emph{Muscle-Mem}~\cite{musclemem}---a verbatim-action-cache baseline with full-CUA fallback on cache miss---shows Muscle-Mem outperforming PreAct's gate-ON warm SR by $\sim 3.25$ tasks on average (Muscle-Mem warm-$\Delta = -0.75 \pm 1.50$ vs.\ PreAct gate-ON $-4.0 \pm 2.94$; one-tailed Welch $p \approx 0.05$). The mechanism is asymmetric fallback: Muscle-Mem's cache-miss falls through cleanly to a fresh CUA run, whereas PreAct's gate-reject leaves Run~2 with no artifact to replay. The harness is the right design where compile artifacts must withstand drift (Android, OSWorld); on byte-identical-replay benchmarks (WebArena), giving the harness an explicit cache-miss-to-CUA path would likely close the gap. The harness---not prompts or guardrails---is the contribution.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Head-to-head baselines on WebArena shopping\_admin 12-task subset, $n{=}4$ reps each. PreAct rows reproduced from Table~\ref{tab:gate-android}'s WebArena column / \S\ref{sec:eval-webarena}.}
\label{tab:baselines}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\textbf{PreAct gate-ON (verify gate enabled)} & $6.0 \pm 0.82$ & $2.0 \pm 2.31$ & $-4.0 \pm 2.94$ & Gate rejects $\approx 83\%$ of compiles; bimodal warm SR (4/4/0/0) \\
\bottomrule
\end{tabular}
\end{table}

The honest reading of Table~\ref{tab:baselines}: at matched $n{=}4$, \textbf{Muscle-Mem outperforms PreAct gate-ON on this benchmark}. Muscle-Mem's warm-SR mean ($6.25/12 = 52\%$) is more than $3\times$ PreAct gate-ON's ($2.0/12 = 17\%$); the warm-$\Delta$ gap is $-0.75$ vs.\ $-4.0$, i.e.\ Muscle-Mem loses about $3.25$ fewer tasks than PreAct does on the cold-to-warm transition. A two-sample Welch's $t$-test on the 4-vs-4 warm-$\Delta$ comparison yields $t \approx 2.0$, $p \approx 0.05$ (one-tailed), so the effect is at the threshold of significance at $n{=}4$ but the direction is unambiguous: every rep favors or ties Muscle-Mem on warm SR. Workflow-Use, by contrast, behaves like PreAct gate-OFF (warm SR $0/12$ in every rep)---it stores compiled scripts without verification and, like gate-OFF, suffers complete warm-time collapse when the scripts encounter live-page selectors that don't match.

\textbf{Mechanism: cache miss vs.\ verify-reject.} WebArena's two-run protocol replays the \emph{same task} on Run~2, so the relevant question is what happens when the stored artifact does not match the live page. Muscle-Mem's blind-replay attempt fails fast (typical $\sim 30$\,s before timing out on a missing selector), and the harness then falls through to a fresh CUA run that re-solves the task with the original LLM cost. PreAct's verify-before-store gate, by contrast, runs an independent verify-replay between Run~1 and Run~2; on these WebArena tasks the compiled state-machine predicates are brittle enough (e.g.\ \texttt{state=reviews\_index\_page} matched via XPath patterns) that they fail re-verification even on the cold-pass task, so the gate deletes the program and Run~2 has no artifact to fall back to. \emph{Muscle-Mem's advantage on WebArena is not that its replay is more reliable---it isn't, with reps showing cache miss rates of $50$--$67\%$---but that its cache-miss path falls through cleanly to CUA, whereas PreAct's gate-reject path leaves no artifact for Run~2 at all.}

\textbf{Implication.} The two designs make different bets: PreAct bets that compile-time verification matters more than coverage; Muscle-Mem bets that even partial-cache + full-CUA-fallback is better than a verified-or-empty corpus. Our $n{=}5{+}5$ cross-platform results indicate PreAct's bet pays off on Android and OSWorld (Tables~\ref{tab:gate-android}, \ref{tab:gate-osworld}: gate-ON $\Delta = +1.2$ and $+0.2$ vs.\ gate-OFF $-1.4$ and $-2.4$, where the protective value of rejecting lossy compiles dominates). On WebArena's same-task replay protocol with brittle answer-extraction predicates, the $n{=}4$ head-to-head says the opposite. We treat this as evidence that PreAct's harness as currently implemented is the right design for stateful UI tasks where compile artifacts must withstand drift, and the wrong design for byte-identical-replay benchmarks where a Muscle-Mem-style cache-miss-then-CUA pipeline is dominant. A productive direction (\S\ref{sec:conclusion}) is to give the harness an explicit cache-miss path on Run~2 when no verified program exists, matching Muscle-Mem's fallback semantics without abandoning the gate on Android/OSWorld.

\subsection{Compile-LLM Robustness: Gemini vs Claude Compile (Preliminary)}
\label{sec:eval-compile-llm}

\emph{This subsection reports a single-rep preliminary observation; readers should weight it accordingly.} To probe whether the verify-gate's compile-rejection rate is Claude-specific or a property of the compile step itself, we wired \texttt{PREACT\_COMPILE\_PROVIDER=gemini}, which swaps the compile-step LLM from Claude Sonnet 4.6 to Gemini 3 Flash (the CUA loop and selector remain Claude). We ran one WebArena rep ($n{=}1$) on the same 12-task shopping\_admin subset:
...
\subsection{Out-of-Distribution Generalization (Preliminary)}
\label{sec:eval-ood}

We tested whether the corpus transfers beyond seen tasks by running a test\_tiny-built rag\_db on the 33 non-test\_tiny tasks of OSWorld test\_small ($n{=}1$ rep). The result is \textbf{approximately neutral}: warm-OOD 21/30 = 70\% vs cold-OOD $\approx 20/30 = 67\%$, a 1-task difference well within the cold-run-to-run noise observed at $n{=}5$ on test\_tiny ($\pm 0.45$ tasks). On this single-pass evidence the corpus appears neither to meaningfully help by transferring abstractions across task families nor to meaningfully hurt by retrieving inapplicable programs; the selector tends to return ``no candidate'' when the OOD task's description does not match a stored program. This tentatively suggests the scope of the ``self-extending corpus'' claim: \textbf{the corpus's value is in repeated invocations of seen tasks rather than cross-task generalization}, but a multi-rep replication is necessary to confirm whether the OOD effect is truly neutral or merely small-and-positive (full breakdown in Appendix~E).

\subsection{Limitations}

Beyond the eight validity threats closed in Table~\ref{tab:gap-closure}, we explicitly acknowledge five remaining limitations:
\begin{itemize}[leftmargin=2em]\tightlist
...
PreAct demonstrates that a \emph{verified self-extending executable code corpus} produces monotonic refinement on multi-platform CUA benchmarks. The harness---RAG retrieval, verify-before-store gate, agentic program selector, hybrid replay, and CUA fallback---is the contribution: a cross-platform 2$\times$2 ablation at $n{=}5{+}5{+}4$ across three structurally different platforms (AndroidWorld, OSWorld, WebArena) shows the gate is empirically necessary for monotonicity, with reproducible cov$=$100\%/score$=$0 replays documenting the lossy-compile pattern it filters and a cross-platform meta-test $p \approx 10^{-4}$ under the no-effect null. The \emph{negative} findings are equally informative: code-level runtime guardrails are aggregate-neutral, and prompt-level Pre-Act content is dead code in production. The harness is what works; the runtime add-ons are not.
...
Future work falls in four directions:

\textbf{Cache-miss-to-CUA fallback on the gate path.} The Muscle-Mem head-to-head (\S\ref{sec:eval-baselines}) identifies a specific gap: when the verify-gate rejects a compile, Run~2 is left with no artifact and falls through to a placeholder (currently returning \texttt{no\_artifact}/score~0). Muscle-Mem's better warm SR on WebArena traces directly to the fact that its cache-miss path runs a fresh CUA on Run~2 and re-solves the task. Adding the same fallback to PreAct's harness---``if Run~1 was a gate-rejected compile, run a fresh CUA on Run~2 too''---would likely close the $-0.75$ vs.\ $-4.0$ warm-$\Delta$ gap on WebArena without sacrificing the gate's Android/OSWorld advantage. This is a small implementation change with an empirically validated motivation.
