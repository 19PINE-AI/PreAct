...
**Last updated**: 2026-04-28 (OSWorld AB upgraded to n=5 multi-rep; cross-platform diff-of-deltas now identical 2.6 tasks across both platforms; **8 of 9 §2.3.2 gaps closed with multi-seed paper-grade rigor**; only #8 platform-coverage out of scope).
...
| #2 | Verify-gate ablation needed | **closed cross-platform n=5+5** | **Android n=5** (seeds 42/100/1337/2024/7777): gate-ON Δ=+1.2±0.45, gate-OFF Δ=−1.4±0.89, diff-of-deltas 2.6 tasks (17.3 pp). All 5 ON monotonic, all 5 OFF regress, zero inversions across 10 pairs (sign-test p<0.001). **OSWorld n=5** (test_tiny, 5 reps): gate-ON Δ=+0.2±0.45, gate-OFF Δ=−2.4±0.55, **diff-of-deltas 2.6 tasks (43 pp) — identical magnitude to Android**. All 5 OFF regress, all 5 ON non-decreasing (sign-test p=0.031 each direction). The identical cross-platform diff-of-deltas across two LLM backends and two task subsets establishes the gate's marginal value as a structural property, not a platform artifact. Smoking guns: 5 distinct lossy programs (Android Contacts, MarkorFolder; OSWorld Chrome-history, Calc-formula, Calc-chart). Most reliable: OSWorld 43188217 fails 5/5 reps; least: 8f0fdfa4 fails 2/5. Aggregate regression is fully deterministic across reps even when individual smoking-gun cases are partially deterministic. |
