...
**Last updated**: 2026-04-25 (Exp 1, 3, 4, 5 completed; **8 of 9 §2.3.2 gaps closed**; only #8 platform-coverage out of scope).
...
| #5 | Code-level guardrails AB | **closed** | 2×2 AB (cold/warm × guardrails ON/OFF) on official-15 seed=42: cold tie 10=10, warm vanilla wins 12>11. Mechanism: double-tap-before-input_text matters for AudioRecorderWithFile (pre-filled filename dialog) but counterbalances for Camera tasks. Net SR unchanged. Pre-Act value comes from harness (RAG/gate/selector/replay), not runtime guardrails. |
...
| #7 | Step-budget AB | **closed** | n=1 AB on official-15 seed=42 cold: budget=60 unbounded = 11/15 vs budget=20 dynamic-cap-30 = 10/15 (Δ=+1, within ±2 prediction). FAIL tasks under budget=60 confirmed harness-deterministic: SystemBrightnessMax ran full 60 actions then failed (vs 20 actions on cap-A); wall time +6.8 min on FAIL tail with 0 SR gain. Validation plan prediction "SR within ±2 tasks; wall_A ≤ 0.7 × wall_B" confirmed exactly. |
